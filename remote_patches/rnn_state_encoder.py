...
class NoMemoryStateEncoder(nn.Module):
    r"""Feed-forward replacement for recurrent state encoders.

    This keeps the policy interface compatible with recurrent policies while
    removing temporal memory. It is useful for no-spatiotemporal-memory
    ablations: each timestep is encoded independently from the current
    observation and previous action only.
    """
    ...
    def forward(
        ...
        return self.fc(x), hidden_states
...
def build_rnn_state_encoder(
    ...
    if rnn_type in {"none", "no_memory", "identity", "mlp"}:
        return NoMemoryStateEncoder(input_size, hidden_size)
    elif rnn_type == "gru":
