...
class PPOTrainer(BaseRLTrainer):
    ...
    def load_checkpoint(self, checkpoint_path: str, *args, **kwargs) -> Dict:
        ...
        """
        # Checkpoints produced by Habitat contain OmegaConf config objects.
        # PyTorch 2.6+ defaults torch.load(..., weights_only=True), which
        # rejects those objects. These checkpoints are produced locally by this
        # experiment, so loading the full trusted checkpoint is intentional.
    ...
    def train(self) -> None:
        ...
        count_checkpoints = 0
        prev_time = 0
        fixed_ckpt_interval = int(
            os.environ.get("HABITAT_CKPT_INTERVAL_FRAMES", "0") or 0
        )
        next_fixed_ckpt_step = (
            ((self.num_steps_done // fixed_ckpt_interval) + 1)
            * fixed_ckpt_interval
            if fixed_ckpt_interval > 0
            else None
        )
        ...
        if rank0_only() and fixed_ckpt_interval > 0:
            logger.info(
                f"Saving checkpoints every {fixed_ckpt_interval} frames "
                f"starting at {next_fixed_ckpt_step}"
            )
            if count_checkpoints == 0 and self.num_steps_done == 0:
                ...
                logger.info("Saved initial fixed-frame checkpoint ckpt.0.pth")
                count_checkpoints += 1
        ...
        with (
            ...
            while not self.is_done():
                ...
                if rank0_only():
                    if fixed_ckpt_interval > 0:
                        assert next_fixed_ckpt_step is not None
                        while self.num_steps_done >= next_fixed_ckpt_step:
                            self.save_checkpoint(
                                f"ckpt.{count_checkpoints}.pth",
                                dict(
                                    step=self.num_steps_done,
                                    target_step=next_fixed_ckpt_step,
                                    wall_time=(time.time() - self.t_start)
                                    + prev_time,
                                ),
                            )
                            logger.info(
                                "Saved fixed-frame checkpoint "
                                f"ckpt.{count_checkpoints}.pth at "
                                f"{self.num_steps_done} frames "
                                f"(target {next_fixed_ckpt_step})"
                            )
                            count_checkpoints += 1
                            next_fixed_ckpt_step += fixed_ckpt_interval
            ...
            if rank0_only():
                ...
                logger.info(f"Saved final checkpoint at {self.num_steps_done} frames")
