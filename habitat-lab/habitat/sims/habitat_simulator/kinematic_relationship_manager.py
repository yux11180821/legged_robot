...
class KinematicRelationshipManager:
    ...
    def initialize_from_obj_to_rec_pairs(
        ...
        for obj_handle, rec_unique_name in obj_to_rec.items():
            ...
            if obj is None:
                logger.warn(
                    f"Cannot find object {obj_handle}, so cannot create a "
                    "kinematic parent relationship. Skipping. Note that "
                    "episode object metadata may be inconsistent with the "
                    "loaded object handles."
                )
                continue
