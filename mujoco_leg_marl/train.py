...
"""Train the Go2 per-leg MARL lower layer (4 legs = 4 agents, CTDE MAPPO).

    python mujoco_leg_marl/train.py --num-envs 32 --total-steps 3000000 --seed 100

Reuses the project's bug-fixed CTDE PPO core (truncation-aware GAE + LR annealing
+ KL early-stop), tanh action distribution, and metrics harness.  Only the MuJoCo
env + per-leg actor are env-specific.  Outputs metrics CSV (every --log-every
env-steps, incl. single-step inference time) and checkpoints.
"""
...
from mujoco_leg_marl.go2_leg_env import (  # noqa: E402
    LEG_NAMES, PER_LEG_OBS_DIM, STATE_DIM, Go2LegMARLCfg, Go2LegMARLEnv,
)
from mujoco_leg_marl.policy import CentralCritic, LegActor  # noqa: E402


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Go2 per-leg MARL (MuJoCo).")
    p.add_argument("--model-path", default="mujoco_menagerie/unitree_go2/scene.xml")
    p.add_argument("--exp-name", default="go2_leg_marl")
...
def main() -> None:
    ...
    cfg = Go2LegMARLCfg(model_path=args.model_path, num_envs=N)
    env = Go2LegMARLEnv(cfg)
    env.rng = np.random.default_rng(args.seed)

    actor = LegActor(PER_LEG_OBS_DIM, action_dim=3, hidden_size=args.hidden_size).to(device)
    critic = CentralCritic(STATE_DIM, hidden_size=args.hidden_size).to(device)
    ...
    logger = StepLogger(results_dir / f"legmarl_seed_{args.seed}_metrics.csv", log_every=args.log_every,
    ...
    print(f"go2_leg_marl start envs={N} agents={A} obs={PER_LEG_OBS_DIM} state={STATE_DIM} device={device}",
          flush=True)
    while frames < args.total_steps:
        b_obs, b_raw, b_lp, b_state, b_rew, b_done, b_boot, b_val = [], [], [], [], [], [], [], []
        for _ in range(args.rollout_steps):
            with timer.measure():
                with torch.no_grad():
                    raw, env_a, lp, _ = actor.act(to_t(exo.reshape(N * A, PER_LEG_OBS_DIM)), None)
                    val = critic(to_t(state))
            actions = env_a.cpu().numpy().reshape(N, A, 3)
            nxt_exo, nxt_state, reward, done, info = env.step(actions)
            team_rew = reward.mean(axis=1)  # MAPPO shared-reward (team)
            ep_ret += team_rew
            ep_len += 1
            ...
            if len(trunc):
                with torch.no_grad():
                    boots[trunc] = critic(to_t(info["term_state"][trunc])).cpu().numpy()
            for e in np.nonzero(done)[0]:
                recent_ret.append(float(ep_ret[e])); recent_len.append(float(ep_len[e]))
                ep_ret[e] = 0.0; ep_len[e] = 0

            b_obs.append(exo.reshape(N * A, PER_LEG_OBS_DIM).copy())
            b_raw.append(raw.cpu().numpy()); b_lp.append(lp.cpu().numpy())
            b_state.append(state.copy()); b_rew.append(team_rew.copy())
            b_done.append(done.astype(np.float32)); b_boot.append(boots)
            b_val.append(val.cpu().numpy())

            exo, state = nxt_exo, nxt_state
            frames += N
        ...
        with torch.no_grad():
            last_value = critic(to_t(state)).cpu().numpy()
        ...
        agent_adv = broadcast_team_advantage(team_adv, A)  # [T,N,A]
        ...
        if not args.no_anneal_lr:
            ...
            for g in optimizer.param_groups:
                g["lr"] = lr_now
        batch = MARLBatch(
            actor_obs=to_t(np.asarray(b_obs, np.float32)).flatten(0, 1),
            actor_hidden=None,
            raw_actions=to_t(np.asarray(b_raw, np.float32)).flatten(0, 1),
            old_log_probs=to_t(np.asarray(b_lp, np.float32)).flatten(0, 1),
            advantages=normalize_advantages(to_t(agent_adv.reshape(-1))),
            critic_obs=to_t(np.asarray(b_state, np.float32)).flatten(0, 1),
            returns=to_t(team_ret.reshape(-1)))
        losses = marl_ppo_update(actor, critic, optimizer, batch,
        ...
        if frames >= next_ckpt:
            ...
            torch.save({"actor": actor.state_dict(), "critic": critic.state_dict(),
                        "frames": frames, "seed": args.seed, "cfg": vars(cfg)},
                       ckpt_dir / f"legmarl_seed_{args.seed}_{frames:09d}.pt")
            next_ckpt += args.ckpt_interval

    ckpt_dir.mkdir(parents=True, exist_ok=True)
    torch.save({"actor": actor.state_dict(), "critic": critic.state_dict(),
                "frames": frames, "seed": args.seed, "cfg": vars(cfg)},
               ckpt_dir / f"legmarl_seed_{args.seed}_final.pt")
    print(f"go2_leg_marl done frames={frames}", flush=True)
